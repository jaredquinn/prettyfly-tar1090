...
stats = CraftStat(WORDS, LANG)
cache = RegoCache(CONFIG)

doAll=False
endTime = datetime.datetime.now()
startTime = datetime.datetime.now()-datetime.timedelta(hours=HOURS)

with open(os.path.join(CONFIG.get('RUN'), 'chunks.json'), 'r') as f:
      data = json.load(f)

      for item in data.get('chunks_all', []):
          with gzip.open(os.path.join(CONFIG.get('RUN'), item), 'rb') as f_in:
              ...
              for part in parts:
                  ...
                  if doAll==True or (now > startTime and now < endTime):
                      for craft in part.get('aircraft',[]):
                          objCraft = IncomingCraft(craft, CONFIG)
...
for k,v in data:
    if k is not None:
        print('%s %d: %s' % ( WORDS.get(k)[LANG], len(v), ', '.join([ '%s/%s' % (i[1],i[0]) for i in v ]) ) )
...
for k in sorted(data, key=lambda k: len(data[k]), reverse=True):
    ...
    if len(data[k]) > 0:
        op = stats.OPERATORS.get('callsigns').get(k)
        ln = stats.OPERATORS.get('classes').get(op.get('class'), {})
        em = ln.get('emoji', '🛫')
        lab = k
        info = 'callsigns'
        if lab[0] in ['*', '?']:
